import { signal, computed, $ } from "@builder.io/qwik";
...
const progressStore = signal<Record<string, Record<string, NodeProgress>>>({});
...
export const saveProgressToStorage = $(() => {
  ...
  try {
    localStorage.setItem(STORAGE_KEY, JSON.stringify(progressStore.value));
  } catch (error) {
 ...
 */
export const getNodeProgress = $(
  (roadmapId: string, nodeId: string): NodeProgress | null => {
    return progressStore.value[roadmapId]?.[nodeId] || null;
  }
);
 ...
 */
export const setNodeProgress = $(
  (roadmapId: string, nodeId: string, status: ProgressStatus, notes?: string) => {
 ...
 */
export const cycleNodeProgress = $(
...
export const getRoadmapProgress = $(
  (roadmapId: string): Record<string, NodeProgress> => {
    return progressStore.value[roadmapId] || {};
  }
);
...
export const computeProgressStats = $(
  (roadmapId: string, totalNodes: number) => {
    ...
    try {
      ...
      if (merge) {
        ...
        for (const [roadmapId, roadmapProgress] of Object.entries(data.data)) {
          ...
          Object.assign(current[roadmapId], roadmapProgress);
        }
        progressStore.value = { ...current };
      } else {
        // Replace all progress
        progressStore.value = data.data;
      }
 ...
 */
export const getProgressStatusInfo = $(
  (status: ProgressStatus) => {
    switch (status) {
      case 'todo':
        return {
...
/**
 * Computed signal for progress store (for reactive updates)
 */
export const progressSignal = computed$(() => progressStore.value);
...
// Export the raw signal for advanced usage
